...
def extract_feature(model, test_loader, dev):
    model.eval()
...
market_data = Market3D(opt.dataset_path, flip=False, slim=opt.slim, norm =opt.norm, erase=0, channel = opt.channel, bg = opt.bg)
...
if opt.use_dense:
    model = Model_dense(opt.k, opt.feature_dims, [512], output_classes=opt.class_num, init_points = opt.init_points, input_dims=3, npart = opt.npart, id_skip = opt.id_skip, res_scale = opt.res_scale, light=opt.light, cluster = opt.cluster, conv = opt.conv,  use_xyz = not opt.no_xyz)
elif opt.use_dense2:
...
# Extract feature
with torch.no_grad():
    query_feature = extract_feature(model, query_loader, dev)
    gallery_feature = extract_feature(model, gallery_loader, dev)
